...
"
    ...
    Public Function GetSqlCreateSystemDummyViewProcedure() As String Implements IDBSqlGenerator.GetSqlCreateSystemDummyViewProcedure
        Dim ret As String = "
IF OBJECT_ID('DBTimeLine.CreateDummyView') IS NOT NULL
    DROP PROCEDURE DBTimeLine.CreateDummyView
GO
CREATE PROCEDURE DBTimeLine.CreateDummyView 
...
AS
BEGIN
	IF OBJECT_ID(@SchemaName + '.' + @Name) IS NULL
	BEGIN
		DECLARE @sql varchar(max)
		SET @sql = N'CREATE VIEW ' + @SchemaName + '.' + @Name + 
		N' AS 
		( 
			SELECT Dummy = 1
		)'
...
"
    ...
    Public Function GetSqlCreateSystemDummyProcedureProcedure() As String Implements IDBSqlGenerator.GetSqlCreateSystemDummyProcedureProcedure
        Dim ret As String = "
IF OBJECT_ID('DBTimeLine.CreateDummyProcedure') IS NOT NULL
    DROP PROCEDURE DBTimeLine.CreateDummyProcedure
GO
CREATE PROCEDURE DBTimeLine.CreateDummyProcedure 
(
	@SchemaName NVARCHAR(MAX),
	@Name NVARCHAR(MAX)
)
...
BEGIN
	...
	BEGIN
		DECLARE @sql varchar(max)
		SET @sql = N'CREATE PROCEDURE ' + @SchemaName + '.' + @Name + 
		N' AS 
		( 
			SELECT Dummy = 1
		)'
